...
def getTOfflineCabang(TOffline, branchCode):
	...
	for i in range(0, len(TOfflineKanca)):
		if TOfflineKanca[i][2] == 0:
			...
			TOfflineUKO.append((TOfflineKanca[i][3], TID, durasiLastTunai(getLastTunai(TID))))

	for i in range(0, len(TOfflineKanca)):
		if TOfflineKanca[i][2] == 1:
			TID = TOfflineKanca[i][5]
			TOfflineCRO.append((TOfflineKanca[i][1], TOfflineKanca[i][4], TID, durasiLastTunai(getLastTunai(TID))))
	...
	if TOfflineUKO:
		...
		for i in range(0, len(TOfflineUKO)):
					msgBody += str(i+1)+" "+ str(TOfflineUKO[i][0])+", "+str(TOfflineUKO[i][1])+", "+(str(TOfflineUKO[i][2]))+"\n"
		msgBody += "\n"			

	if TOfflineCRO:
		msgBody += "*[CRO]*\n"
		for i in range(0, len(TOfflineCRO)):
					msgBody += str(i+1)+" "+ str(TOfflineCRO[i][0]) +": "+str(TOfflineCRO[i][1])+", "+str(TOfflineCRO[i][2])+", "+(str(TOfflineCRO[i][3]))+"\n"
...
def getTOfflineCRO(TOffline, selectedCRO):
	...
	if selectedCRO == 0:
		...
		for i in range(0, len(TOffline)):
			if TOffline[i][2] == 1:
				strNamaCabang = "*"+cleanupNamaUker(TOffline[i][-1])+"*"
				TID = TOffline[i][5]
				TOfflineCRO.append((TOffline[i][1], TOffline[i][4], TID, durasiLastTunai(getLastTunai(TID))))
	...
	if TOfflineCRO:
		...
		TOfflineCRO = sorted(TOfflineCRO, key=itemgetter(0,3), reverse = True)
		msgBody += strCRO+"\n"
		for i in range(0, len(TOfflineCRO)):
			...
			for j in range(0, len(arrCRO)):
				...
				if str(TOfflineCRO[i][0]) == arrCRO[j]:
					seqNo += 1
					counter += 1
					msgBody += str(seqNo)+") "+ str(TOfflineCRO[i][1])+", "+str(TOfflineCRO[i][2])+", "+str(TOfflineCRO[i][3])+"\n"
...
def getTOfflineUKO(TOffline):
	...
	for i in range(0, len(TOffline)):
		...
		if TOffline[i][2] == 0:
			strNamaCabang = "*"+cleanupNamaUker(TOffline[i][-1])+"*"
			TID = TOffline[i][5]
			TOfflineUKO.append((strNamaCabang.upper(), TOffline[i][4], TID, durasiLastTunai(getLastTunai(TID))))

	if TOfflineUKO:
		TOfflineUKO = sorted(TOfflineUKO, key=itemgetter(0,3), reverse = True)
		msgBody += "*[UKO]*\n"
		for i in range(0, len(TOfflineUKO)):
			...
			seqNo += 1
			counter += 1
			msgBody += str(seqNo)+") "+ str(TOfflineUKO[i][1])+", "+str(TOfflineUKO[i][2])+", "+(str(TOfflineUKO[i][3]))+"\n"
...
def getTOfflineACI(TOffline, selectedACI):
	...
	for i in range(0, len(TOffline)):
		...
		if selectedACI in TOffline[i][4]:
			...
			TOfflineACI.append((strNamaCabang.upper(), strLocation, TID, durasiLastTunai(getLastTunai(TID))))

	seqNo = 0

	if TOfflineACI:

		TOfflineACI = sorted(TOfflineACI, key=itemgetter(0, 3), reverse = False)
		...
		for i in range(0, len(TOfflineACI)):
			...
			seqNo += 1
			counter += 1
			msgBody += str(seqNo)+") "+ str(TOfflineACI[i][1])+", "+str(TOfflineACI[i][2])+", "+durasiLastTunai(str(TOfflineACI[i][3]))+"\n"
